...
seq_planner_class::seq_planner_class(string robotack,string simures):simulationtopicnamce_(simures),
robotacktopicname_(robotack){
    cout<<BOLD(FBLU("seq_planner_class::seq_planner_class"))<<endl;
   
    subHumanActionAck=nh.subscribe("human_action",100, &seq_planner_class::CallBackHumanAck, this);
	subRobotActionAck=nh.subscribe(robotacktopicname_,100, &seq_planner_class::CallBackRobotAck, this);
	subSimulationAck=nh.subscribe(simulationtopicnamce_,100, &seq_planner_class::UpdateSimulation, this);

	pubRobotCommand = nh.advertise<std_msgs::String>("robot_command",1);
	pubSimulationCommand=nh.advertise<robot_interface_msgs::SimulationRequestMsg>("simulation_command",1);
...
void seq_planner_class::loadFiles(string seqPlannerPath,string AssemblyName){


	


	cout<<BOLD(FBLU("seq_planner_class::loadFiles"))<<endl;
	 char* DataLogPath	="/home/hossein/catkin_ws/src/TAMP/plannerLog";
	string DataLogPathStr (DataLogPath);
	mkdir(DataLogPath , S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
	fileLog.open((DataLogPathStr+"/tamplog.txt").c_str());
//	simFileLog.open((DataLogPathStr+"/1_SimulationFileLog.txt").c_str(),ios::app);
    ...
    cout<<FRED(BOLD("Loading Files of Planner......"))<<assembly_name<<endl;
...
//												cout<<"The agent: "<<AgentsName[n]<<"could not perform action:"<<state_action_table[i].actions_list[j].name <<endl;
										...
										{
											if(state_action_table.size()==1)
											{
												 //state_action_table[i].actions_list[j].assigned_agents.size()==1
...
//			cout<<"551: "<<is_a_state_solved<<is_a_state_feasible<<is_an_action_done<<endl;
			...
			else
			{
				...
				//exit(1);
				//resetplanner();
				cout<<BOLD(FRED("Restting Object planner"))<<endl;
                   tamp_msgs::resetsrv msg;
         	msg.request.reset = true;
         	msg.request.planname=assembly_name;
         	//inflow_ = false;
         	//if(!inflow_){
         		  if(resetClient.call(msg)){
            	ROS_INFO("resseiting andor graph");
            }
         	//}
...
void seq_planner_class::FindOptimalState(void){
	...
	for(int i=0;i<state_action_table.size();i++)
	{
		if(state_action_table[i].isFeasible==true && state_action_table[i].andorNameMain==hierarchicalGraphList.back())
		{
			...
			{	cout<<"state name: "<<state_action_table[i].state_name <<" ,state cost: "<<state_action_table[i].state_cost<<endl;
...
void seq_planner_class::FindNextAction(){
	...
	else
	{
		...
		if(assembly_name=="normal"){
			lastOptimalState_ = state_action_table[optimal_state].state_name;
		}
...
//		state_action_table[i].PrintSummary()();
	...
	if(graphSolved==true) // it is an complex action
	{
		...
		{	
			//inflow_=false;
			cout<<BOLD(FGRN("The task planning is Done for: "))<<assembly_name<<endl;
			cout<<BOLD(FRED("Restting Object planner"))<<endl;
			   tamp_msgs::resetsrv msg;
         	msg.request.reset = false;
         	msg.request.planname=assembly_name;
         	//if(!inflow_){
         		if(resetClient.call(msg)){
            	ROS_INFO("resseiting andor graph");
...
void seq_planner_class::CheckStateExecution(){
	...
	if(state_action_table.size()==0){
		//inflow_=false;
		cout<<FRED("There is no feasible state to be checked for planner: " )<<assembly_name<<endl;
		//exit(1);
		cout<<BOLD(FRED("Restting Object planner"))<<endl;
           ...
           // inflow_=true;
            updateAndor=true;
            //cout<<BOLD(FRED("------------- RESET TASK PLANNER -------------# "))<<nutp<<endl;
         	//plan_obj.~seq_planner_class(); // destruct
			//new(&plan_obj) seq_planner_class(assemblyPath, AssemblyName); 
...
//	cout<<"203"<<endl;
	...
	{    cout<<FBLU("Time66: ")<<to_string(ros::Time::now().toSec())<<endl;
...
void seq_planner_class::SetActionDefinitionList(string actionDefinitionPath){
	...
	//    }
	 cout<<"file_path_ifStr.is_open()*********99"<<endl;
}
void seq_planner_class::SetAgentsList(void){
	...
	//agent[1]: LeftArm
	agent tempAgent1;
	tempAgent1.name="RightArm";
	tempAgent1.type="Robot";
	agents.push_back(tempAgent1);

	//agent[2]: RightArm
	agent tempAgent2;
	tempAgent2.name="LeftArm";
	tempAgent2.type="Robot";
	agents.push_back(tempAgent2);

	agent tempAgent3;
	tempAgent3.name="Base";
	tempAgent3.type="Robot";
	agents.push_back(tempAgent3);
...
void seq_planner_class::SetStateActionList(string stateActionPath, string andorGraphName, vector<offline_state_action> & offline_state_action_list){
	...
	vecofstac_ = offline_state_action_list;

}

std::vector<string> seq_planner_class::getActionforState(string & statename){
    std::size_t state_index;
    std::vector<string> actionsvector;
    for(std::size_t i=0;i<vecofstac_.size();i++){
        
        if(vecofstac_[i].state_name==statename){
        	state_index =i;
        	break;
        }
    ...
    for(std::size_t i=0;i<vecofstac_[state_index].actions_list.size();i++){

        actionsvector.push_back(vecofstac_[state_index].actions_list[i].actionAndParameters);
    }
...
void seq_planner_class::CallBackHumanAck(const std_msgs::Int16::ConstPtr& msg){
	cout<<BOLD(FBLU("seq_planner_class::CallBackHumanAck"))<<endl;
    string actionName;
	int actionId=msg-> data;
	...
	cout<<BOLD(FGRN("Human Sent Action: "))<<actionName<<endl;
	if(actionName=="PutDown" || actionName=="PickUp" || actionName=="Cuff" || actionName=="UnCuff" ||actionName=="insert" ||actionName == "askhuman"){
...
void seq_planner_class::resetplanner(){
    cout<<BOLD(FBLU("Resetting Planner"))<<endl;
