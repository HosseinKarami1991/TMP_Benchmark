...
public:
    seq_planner_class(string robotack="robot_ack",string simures = "simulation_response");
	void loadFiles(string seqPlannerPath,string AssemblyName);
	~seq_planner_class();
    ...
    bool needtoreset_;
    string lastOptimalState_;
    std::vector<string> getActionforState(string & statename);
    bool lastGraphQuery(tamp_msgs::lastgraph::Request &req,tamp_msgs::lastgraph::Response & res);
    bool inflow_,intransition_;
private:
    ...
    vector<offline_state_action> vecofstac_;
    ofstream fileLog;
//    ofstream simFileLog;
    string simulationtopicnamce_,robotacktopicname_;
    double timeNow;
...
//	int agent_update;// the agent number who arrives the latest ack.
	...
	ros::ServiceClient knowledgeBase_client,resetClient;

	void CallBackHumanAck(const std_msgs::Int16::ConstPtr& msg);
